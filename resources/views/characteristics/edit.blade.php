    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                <form id="form" name="form" action="{{route('characteristic.update', $characteristic->id)}}" method="post">
                    @csrf
                    @method('PUT')
                    <div class="form-group row">
                        <div class="col-sm-2">
                            ...
                            <?php
                                $year = $characteristic->name;
                            ?>
                            @for($i=$year-5; $i <$year+5; $i++)
                                <option value="{{$i}}" {{ $characteristic->name == $i ? "selected" : "" }}>{{$i}}</option>
                            @endfor
                            </select>
                            @error('category')
                        ...
                        <div class="col-sm-10">
                            ...
                            <textarea class="form-control  @error('description') is-invalid @enderror"  id="description" name="description" >{{ old('description') ?? $characteristic->description }}</textarea>
                            @error('description')
                                <span class="invalid-feedback" role="alert">
                                    {{ $message }}
                                </span>
